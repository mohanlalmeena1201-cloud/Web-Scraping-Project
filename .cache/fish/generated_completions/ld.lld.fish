# ld.lld
# Autogenerated from man page /data/data/com.termux/files/usr/share/man/man1/ld.lld.1.gz
...
complete -c 'ld.lld' -o Bsymbolic-non-weak -d 'Bind default visibility defined STB_GLOBAL symbols locally for shared'
...
complete -c 'ld.lld' -l be8 -d 'Write a Big Endian ELF File using BE8 format(AArch32 only)'
complete -c 'ld.lld' -l branch-to-branch -d 'Enable the branch-to-branch optimizations: a branch whose target is another b…'
complete -c 'ld.lld' -l build-id -d 'Generate a build ID note'
complete -c 'ld.lld' -l call-graph-profile-sort -d 'algorithm may be: -tag -width 2n -compact none Ignore call graph profile'
...
complete -c 'ld.lld' -l compress-sections -d 'Compress output sections that match the glob and do not have the SHF_ALLOC fl…'
complete -c 'ld.lld' -l cref -d 'Output cross reference table.  If Map is specified, print to the map file'
complete -c 'ld.lld' -l debug-names -d 'Generate a merged . debug_names section'
complete -c 'ld.lld' -l default-script -d 'In the absence of -script, read this default linker script'
...
complete -c 'ld.lld' -l enable-non-contiguous-regions -d 'Spill input sections to later matching output sections to avoid memory region…'
...
complete -c 'ld.lld' -l force-group-allocation -d 'Only meaningful for -r.  Section groups are discarded'
...
complete -c 'ld.lld' -l no-warn-mismatch -d 'Do not reject unknown section types'
...
complete -c 'ld.lld' -s O -d 'Optimize output file'
...
complete -c 'ld.lld' -l package-metadata -d 'Emit a percent-encoded string to the . note. package section'
...
complete -c 'ld.lld' -l randomize-section-padding -d 'Randomly insert padding between input sections and at the start of each segme…'
complete -c 'ld.lld' -l -relax-gp -d 'Enable global pointer relaxation for RISC-V'
complete -c 'ld.lld' -l relocatable -d 'Create relocatable object file'
complete -c 'ld.lld' -l remap-inputs -d 'Input files matching from-glob are mapped to to-file'
complete -c 'ld.lld' -l remap-inputs-file -d 'Remap input files based on patterns in file'
...
complete -c 'ld.lld' -l fat-lto-objects -d 'Use the . llvm'
complete -c 'ld.lld' -l no-fat-lto-objects -d 'Ignore the . llvm. lto section in relocatable object files (default)'
...
complete -c 'ld.lld' -l why-live -d 'Report a chain of references preventing garbage collection for each symbol ma…'
