# editor
# Autogenerated from man page /data/data/com.termux/files/usr/share/man/man1/editor.1.gz
...
complete -c editor -s C -l backupdir -d 'Make and keep not just one backup file, but make and keep a uniquely numbered…'
complete -c editor -s D -l boldtext -d 'For the interface, use bold instead of reverse video'
complete -c editor -s E -l tabstospaces -d 'Convert each typed tab to spaces \\(em to the number of spaces that a tab at t…'
...
complete -c editor -s J -l guidestripe -d 'Draw a vertical stripe at the given column, to help judge the width of the te…'
...
complete -c editor -s T -l tabsize -d 'Set the size (width) of a tab to number columns'
...
complete -c editor -s Y -l syntax -d 'Specify the name of the syntax highlighting to use from among the ones define…'
...
complete -c editor -s b -l breaklonglines -d 'Automatically hard-wrap the current line when it becomes overlong, that is: w…'
complete -c editor -s c -l constantshow -d 'Constantly report the cursor position on the status bar'
...
complete -c editor -s f -l rcfile -d 'Read only this file for setting nano\'s options, instead of reading both the s…'
...
complete -c editor -s o -l operatingdir -d 'Change to the given directory, and allow reading and writing files only in th…'
complete -c editor -s p -l preserve -d 'Preserve the XOFF and XON sequences (^S and ^Q) so that they are caught by th…'
complete -c editor -s q -l indicator -d 'Display a "scrollbar" on the righthand side of the edit window'
complete -c editor -s r -l fill -d 'Set the target width for justifying and automatic hard-wrapping at this numbe…'
...
complete -c editor -s z -l listsyntaxes -d 'List the names of the available syntaxes and exit'
complete -c editor -l magic -d 'When neither the file\'s name nor its first line give a clue, try using libmag…'
complete -c editor -s @ -l colonparsing -d 'When a filename given on the command line ends in a colon plus digits and thi…'
...
complete -c editor -s 1 -l solosidescroll -d 'When the cursor approaches the right edge of the edit window, scroll only the…'
complete -c editor -s / -l modernbindings -d 'Use key bindings similar to the ones that most modern programs use: ^X cuts, …'
