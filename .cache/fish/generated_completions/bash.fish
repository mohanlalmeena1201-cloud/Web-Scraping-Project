...
complete -c bash -s D -d 'Print a list of all double-quoted strings preceded by $ on the standard output'
complete -c bash -l debugger -d 'Arrange for the debugger profile to be executed before the shell starts'
complete -c bash -l dump-po-strings -d 'Equivalent to -D, but the output is in the GNU gettext '
...
complete -c bash -l norc -d 'Do not read and execute the personal initialization file . FN \\*~/'
complete -c bash -l posix -d 'Enable posix mode; change the behavior of bash where the default operation di…'
...
complete -c bash -o conformant -d 'OPTIONS All of the single-character shell options documented in the descripti…'
...
complete -c bash -o mandated -d 'and there is a maximum value (currently 8192) that this may not exceed'
...
complete -c bash -s F -d 'option to declare or typeset lists the function names only (and optionally th…'
...
complete -c bash -s k -d 'True if file exists and its'
...
complete -c bash -s N -d 'True if file exists and has been modified since it was last accessed'
...
complete -c bash -s P -d 'options, respectively, to each completion, and returns the result to readline…'
...
complete -c bash -s q -d 'Display key sequences that invoke the named readline function'
complete -c bash -s X -d 'List all key sequences bound to shell commands and the associated commands in…'
complete -c bash -s E -d and
complete -c bash -s I -d 'and write the matches to the standard output'
complete -c bash -s C -d 'command is executed in a subshell environment, and its output is used as the …'
complete -c bash -s W -d 'Split the wordlist using the characters in the'
complete -c bash -s 1 -d 'and -2 represents the time the shell was invoked'
complete -c bash -s B
complete -c bash -s T
complete -c bash -s H
